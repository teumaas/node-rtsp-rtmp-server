...
(function() {
  ...
  flushRTMPMessages = function(stream, params) {
    ...
    for (k = 0, len2 = allSessions.length; k < len2; k++) {
      ...
      if (((ref = session.stream) != null ? ref.id : void 0) !== stream.id) { // The session is not associated with current stream
        continue;
      }
      msgs = null;
  ...
  createRTMPMessage = function(params, chunkSize = 128) {
    var body, bodyChunk, bodyChunkLen, bodyLength, bufs, extendedTimestampBuf, formatTypeID, timestamp, totalLength, type3Header, useExtendedTimestamp;
    bodyLength = params.body.length;
    ...
    if (params.timestamp >= 0xffffff) {
      ...
      extendedTimestampBuf = new Buffer([(params.timestamp >> 24) & 0xff, (params.timestamp >> 16) & 0xff, (params.timestamp >> 8) & 0xff, params.timestamp & 0xff]);
    } else {
  ...
  RTMPSession = class RTMPSession {
    ...
    respondPlay(commandMessage, callback, streamId = null) {
      ...
      setChunkSize = createRTMPMessage({
        ...
        messageStreamID: 0,
        body: new Buffer([(this.chunkSize >>> 24) & 0x7f, (this.chunkSize >>> 16) & 0xff, (this.chunkSize >>> 8) & 0xff, this.chunkSize & 0xff]) // top bit must be zero
      });
      ...
      if (doPause) { // playing -> pause
        ...
        if (((ref1 = this.stream) != null ? ref1.type : void 0) === avstreams.STREAM_TYPE_RECORDED) {
          ...
          return this.stream.seek(seekMsec / 1000, (err, actualStartTime) => {
            ...
            codecConfigs = this.getCodecConfigs(msec);
            callback(null, this.concatenate([setChunkSize, streamIsRecorded, streamBegin1, unpauseNotify, playStart, rtmpSampleAccess, dataStart, onMetaData, codecConfigs]));
            seq = new Sequent;
            this.startPlaying();
            ...
            return seq.wait(1, () => {
              ...
              if (!isResumed) {
                ...
                return logger.info(`[rtmp:client=${this.clientid}] resumed at ${msec / 1000
            // live
          ...
          for (j = 0, len1 = ref.length; j < len1; j++) {
            ...
            switch (rtmpMessage.messageTypeID) {
              ...
              case 5: // Window Acknowledgement Size
                this.windowAckSize = (rtmpMessage.body[0] << 24) + (rtmpMessage.body[1] << 16) + (rtmpMessage.body[2] << 8) + rtmpMessage.body[3];
                if (DEBUG_INCOMING_RTMP_PACKETS) {
              ...
              case 15: // AMF3 data message
                ...
                if (dataMessage != null) {
                  ...
                  results.push(this.handleAMFDataMessage(dataMessage, function(err, output) {
                    if (err != null) {
              ...
              case 17: // AMF3 command (0x11)
                ...
                results.push(this.handleAMFCommandMessage(commandMessage, function(err, output) {
                  if (err != null) {
                    logger.error(`[rtmp:receive] packet error: ${err}`);
                  }
                  if (output != null) {
    ...
    respondSend(req, callback) {
      ...
      return this.rtmpSession.handleData(req.rawbody, (err, output) => {
        ...
        if (err) {
          ...
          interval = new Buffer([this.pollingDelay]);
