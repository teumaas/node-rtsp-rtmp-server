...
(function() {
    ...
    onOrderedPacket(tag, packet) {
      ...
      if ((match = /^h264:(.*)$/.exec(tag)) != null) {
        ...
        if (packet.h264.fu_a != null) {
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            nalUnit = ref[j];
            results.push(this.onH264NALUnit(clientId, nalUnit, packet, packet.rtpHeader.timestamp)); // single NAL unit
          }
          return results;
        } else {
          return this.onH264NALUnit(clientId, packet.h264.nal_unit, packet, packet.rtpHeader.timestamp); // STAP-A
      // FU-A
        }
  ...
  api = {
    ...
    readH264Payload: function(bits) {
      ...
      if ((1 <= (ref = info.nal_unit_type) && ref <= 23)) { // Single NAL unit packet
        bits.push_back_byte();
