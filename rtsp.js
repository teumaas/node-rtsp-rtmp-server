  ...
  RTSPServer = class RTSPServer {
    ...
    handleTunneledPOSTData(client, data = '', callback) {
      ...
      if (base64Buf.length > 0) {
        ...
        decodedBuf = new Buffer(base64Buf, 'base64'); // no base64 input
      } else {
    ...
    respondSetup(socket, req, callback) {
      var ch1, ch2, client, controlPort, control_ch, dataPort, data_ch, dateHeader, j, len, match, media, mediaType, ref, ref1, res, sdpInfo, serverPort, setupStreamId, ssrc, stream, streamId, target, track, transportHeader, useTCPTransport;
      client = this.clients[socket.clientID];
        ...
        if ((match = /;interleaved=(\d)-(\d)/.exec(req.headers.transport)) != null) {
          ...
          if (mediaType === 'video') {
            ...
            if ((match = /;client_port=(\d+)-(\d+)/.exec(req.headers.transport)) != null) {
              ...
              this.rtspUploadingClients[client.ip + ':' + parseInt(match[2])] = client; // audio
            }
          } else {
