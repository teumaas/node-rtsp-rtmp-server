...
(function() {
  ...
  var AUDIO_CONTROL_PORT, AUDIO_DATA_PORT, INPUT_FILE, SERVER_HOST, STREAM_NAME, UDP_MAX_PACKET_SIZE, VIDEO_CONTROL_PORT, VIDEO_DATA_PORT, aac, audioControlClient, audioDataClient, checkEnd, h264, hybrid_udp, isAudioEnded, isVideoEnded, mpegts, sendAudioStart, sendVideoStart, videoControlClient, videoDataClient;
  ...
  UDP_MAX_PACKET_SIZE = 8000;

  // -- To here
  videoControlClient = new hybrid_udp.UDPClient;

  videoControlClient.maxPacketSize = UDP_MAX_PACKET_SIZE;

  audioControlClient = new hybrid_udp.UDPClient;

  audioControlClient.maxPacketSize = UDP_MAX_PACKET_SIZE;

  videoDataClient = new hybrid_udp.UDPClient;

  videoDataClient.maxPacketSize = UDP_MAX_PACKET_SIZE;

  audioDataClient = new hybrid_udp.UDPClient;
  ...
  */
  ...
  sendVideoStart = function(callback) {
  ...
  sendAudioStart = function(callback) {
  ...
  h264.on('dts_nal_units', function(pts, dts, nalUnits) {
    var buf, concatNALUnit, e, i, isImportant, len, nalUnit, nalUnitType, nalUnitTypes, nalUnitsWithStartCode, payloadSize;
    isImportant = false;
  ...
  videoControlClient.start(VIDEO_CONTROL_PORT, SERVER_HOST, function() {
    return audioControlClient.start(AUDIO_CONTROL_PORT, SERVER_HOST, function() {
      return videoDataClient.start(VIDEO_DATA_PORT, SERVER_HOST, function() {
        return audioDataClient.start(AUDIO_DATA_PORT, SERVER_HOST, function() {
          // ready to start
          return sendVideoStart(function() {
            return sendAudioStart(function() {
              return mpegts.startStreaming(0); // skip 0 milliseconds from the start
            });
          });
        });
      });
